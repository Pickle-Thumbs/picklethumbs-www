...
function initLinkPreviews() {
  ...
  function popIn(cellIdx, src, delay, replacing) {
    const img = cells[cellIdx];
    img.src = src;
    gsap.fromTo(img,
      { scale: replacing ? 0.8 : 0, opacity: replacing ? 0.8 : 0 },
      { scale: 1, opacity: 1, duration: 0.2, ease: "back.out(2)", delay }
    );
  }

  function popOut(cellIdx, delay) {
    const img = cells[cellIdx];
    gsap.to(img, { scale: 0, opacity: 0, duration: 0.15, ease: "power2.in", delay });
  }

  // ringPlants[0] = center, [1] = cross, [2] = corners
  let ringPlants = [null, null, null];

  function tick() {
    const hadCorners = !!ringPlants[2];
    const hadCross = !!ringPlants[1];
    const hadCenter = !!ringPlants[0];

    // Corners: pop out old corners
    if (hadCorners) {
      RINGS[2].forEach((i) => popOut(i, STEP_DELAY * 2));
    }

    // Cross → corners
    if (hadCross) {
      RINGS[1].forEach((i) => popOut(i, STEP_DELAY));
      const oldCross = ringPlants[1];
      RINGS[2].forEach((i) => popIn(i, oldCross, STEP_DELAY * 2, hadCorners));
    ...
    if (hadCenter) {
      ...
      RINGS[1].forEach((i) => popIn(i, oldCenter, STEP_DELAY, hadCross));
